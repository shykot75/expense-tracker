            ...
            <form action="{{ route('expenses.update', $expense) }}" method="POST" class="space-y-6" id="update-form" @submit="validate($event)">
                ...
                <div class="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-6">
                    <div>
                        ...
                        <input type="date" name="expense_date" value="{{ old('expense_date', $expense->expense_date->format('Y-m-d')) }}" onclick="this.showPicker()" class="block w-full rounded-2xl border-slate-100 py-4 px-6 font-bold text-slate-800 bg-slate-50/50">
                    </div>
                    <div>
